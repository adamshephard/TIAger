...
class SegmentationLoader():
    ...
    def __init__(self, image_path, tissue_mask_path, slide_file):
        ...
        self.image = ops.OpenSlide(image_path)
        self.tissue_mask = tifffile.imread(tissue_mask_path)
        print("Images loaded for segmentation")

        # get image info
        dimensions = self.image.level_dimensions[0]
    ...
    def prepare_patching(window_size, mask_size, dimensions, level, tissue_mask):
        ...
        for info in patch_info:
            ...
            tissue_mask_tile = tissue_mask[int(y*2): int(y*2)+h*2, int(x*2): int(x*2) + w*2]
            tissue_mask_tile = cv2.resize(tissue_mask_tile, (w,h), interpolation=cv2.INTER_NEAREST)
    ...
    def __getitem__(self, idx):
        # for patch_info in range(len(self.patch_info)):
        ...
        # # ).squeeze()
        tissue_mask_tile = self.tissue_mask[y:y+h*2, x: x+w*2]
        tissue_mask_tile = cv2.resize(tissue_mask_tile, (w,h), interpolation=cv2.INTER_NEAREST)
        ...
        # #     startX=x, startY=y, width=w, height=h, level=self.level
        # # )
        image_tile = np.asarray(self.image.read_region((x,y), self.level, (w,h)))[...,:3]
    ...
    def process_batch(image_tile, tissue_mask_tile, patch_info):
        ...
        # image = ops.OpenSlide(image_path)
        # tissue_mask = tifffile.imread(tissue_mask_path)

        # tissue_mask_tile = tissue_mask[y:y+h*2, x: x+w*2]
        # tissue_mask_tile = cv2.resize(tissue_mask_tile, (w,h), interpolation=cv2.INTER_NEAREST)
        # tissue_mask_tile = self.tissue_mask.getUCharPatch(
        #     startX=x, startY=y, width=w, height=h, level=self.level
        # ).squeeze()
        ...
        #     startX=x, startY=y, width=w, height=h, level=self.level
        # )
        # image_tile = np.asarray(image.read_region((x,y), level, (w,h)))[...,:3]
